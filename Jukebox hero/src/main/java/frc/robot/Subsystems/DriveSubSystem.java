...
public class DriveSubSystem extends SubsystemBase {
    ...
    public void arcadeDrive(double pov, Boolean leftbutton,boolean rightbutton,double throttle) {
        ...
        {
            rotVel=leftbutton?-0.5:0.5;
        }
        fwdVel= fwdVel*(1+throttle)/2;
        rotVel=rotVel*(1+throttle)/2;
        
        double setVelLeft = (fwdVel + rotVel) ;
        double setVelRight = (fwdVel - rotVel);

        leftMotor.set(setVelLeft);
        rightMotor.set(setVelRight);
    ...
    public void arcadeDrivePID(double pov, Boolean leftbutton,boolean rightbutton,double throttle) {
          double fwdVel=0.0;
        double rotVel=0.0;
        if (pov==0||pov==180){
            fwdVel = pov==0?-1:1;
        ...
        double forwardSpeed= fwdVel*(1+throttle)/2;
       double rotationSpeed=rotVel*(1+throttle)/2;
        double setFwdSpeed = MathUtil.applyDeadband(forwardSpeed, 0.1);
        double setRotSpeed = MathUtil.applyDeadband(rotationSpeed,.1);
    ...
    public Command DriveWithGuitar(DoubleSupplier pov,BooleanSupplier left,BooleanSupplier right,DoubleSupplier throttle ){
        return run(()->arcadeDrive(pov.getAsDouble(), left.getAsBoolean(), right.getAsBoolean(), throttle.getAsDouble()));
    }

        public Command DriveWithGuitarPID(DoubleSupplier pov,BooleanSupplier left,BooleanSupplier right,DoubleSupplier throttle ){
        return run(()->arcadeDrivePID(pov.getAsDouble(), left.getAsBoolean(), right.getAsBoolean(), throttle.getAsDouble()));
    }
